...
class TicketPanel(discord.ui.View):
    ...
    async def btn(self, i, b):
        ...
        e = discord.Embed(title="◈ DOSSIER ◈", description=f"Bonjour {i.user.mention}, le staff arrive.", color=0x5865f2)
...
@bot.command()
async def warn(ctx, m: discord.Member, *, r="Aucune"): await log_and_mp(ctx, "WARN", m, r); await ctx.send(f"⚠️ {m.mention} a reçu un warn.")
@bot.command()
...
@bot.command()
async def unban(ctx, user_id: int): u = await bot.fetch_user(user_id); await ctx.guild.unban(u); await log_and_mp(ctx, "UNBAN", u, "Débanni"); await ctx.send(f"✅ Utilisateur {user_id} débanni.")
@bot.command()
async def tempmute(ctx, m: discord.Member, s: int):
    ...
    await ctx.send(f"🔇 {m.mention} muté {s}s."); await asyncio.sleep(s); [await ch.set_permissions(m, send_messages=None) for ch in ctx.guild.text_channels]
@bot.command()
async def unmute(ctx, m: discord.Member): [await ch.set_permissions(m, send_messages=None) for ch in ctx.guild.text_channels]; await log_and_mp(ctx, "UNMUTE", m, "Manual"); await ctx.send(f"🔊 {m.mention} unmute.")
...
@bot.command()
async def clear_sanctions(ctx, n: int): await bot.get_channel(LOG_CH_ID).purge(limit=n); await ctx.send(f"🧹 {n} logs supprimés.")
@bot.command()
