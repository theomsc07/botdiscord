...
# --- COMMANDES (Avec confirmation + mentions) ---
@bot.command()
...
@bot.command()
async def warn(ctx, m: discord.Member, *, r="Aucune"): await log_and_mp(ctx, "WARN", m, r); await ctx.send(f"⚠️ {m.mention} a reçu un avertissement.")
@bot.command()
async def kick(ctx, m: discord.Member, *, r="Aucune"): await m.kick(reason=r); await log_and_mp(ctx, "KICK", m, r); await ctx.send(f"👢 {m.mention} a été expulsé.")
@bot.command()
async def ban(ctx, m: discord.Member, *, r="Aucune"): await m.ban(reason=r); await log_and_mp(ctx, "BAN", m, r); await ctx.send(f"🔨 {m.mention} a été banni.")
@bot.command()
async def tempmute(ctx, m: discord.Member, s: int):
    ...
    await ctx.send(f"🔇 {m.mention} est muté pour {s} secondes."); await asyncio.sleep(s)
    [await ch.set_permissions(m, send_messages=None) for ch in ctx.guild.text_channels]
@bot.command()
async def unmute(ctx, m: discord.Member): [await ch.set_permissions(m, send_messages=None) for ch in ctx.guild.text_channels]; await log_and_mp(ctx, "UNMUTE", m, "Manual"); await ctx.send(f"🔊 {m.mention} est unmute.")

# Ticket, Clear, Sanctions... (Mêmes fonctions que précédemment)
@bot.command()
...
@bot.command()
async def clear(ctx, n: int): await ctx.channel.purge(limit=n+1); await ctx.send(f"🧹 {n} messages supprimés.")
@bot.command()
async def clear_sanctions(ctx, n: int): await bot.get_channel(LOG_CH_ID).purge(limit=n); await ctx.send("🧹 Logs purgés.")

# --- TICKETS (Panel resté identique) ---
