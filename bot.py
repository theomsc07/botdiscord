...
class DecisionModal(discord.ui.Modal):
    ...
    async def on_submit(self, i: discord.Interaction):
        try:
            ...
            await i.response.send_message("❌ MP bloqué par l'utilisateur", ephemeral=True)
...
# --- COMMANDES DE MODÉRATION ET GRADES ---
@bot.command()
@commands.has_permissions(kick_members=True)
async def warn(ctx, m: discord.Member, *, r="Aucune"):
    e = await send_mod_embed(ctx, "WARN", m, r)
...
async def derank(ctx, m: discord.Member):
    roles_ids = [ROLE_STAFF, R_T, R_C, R_PLUS, R_SENIOR, R_ADMIN]
...
async def del(ctx):
    await ctx.send("⏳ Suppression du salon...")
