...
async def warn(ctx, m: discord.Member, *, r="Aucune"): await send_log("WARN", ctx, m, r); await ctx.send(f"✅ {m.mention} warn.")
@bot.command()
@commands.has_permissions(kick_members=True)
async def kick(ctx, m: discord.Member, *, r="Aucune"): await m.kick(reason=r); await send_log("KICK", ctx, m, r); await ctx.send(f"✅ {m.mention} kick.")
@bot.command()
@commands.has_permissions(ban_members=True)
async def ban(ctx, m: discord.Member, *, r="Aucune"): await m.ban(reason=r); await send_log("BAN", ctx, m, r); await ctx.send(f"✅ {m.mention} banni.")
@bot.command()
@commands.has_permissions(ban_members=True)
async def unban(ctx, uid: int): u = await bot.fetch_user(uid); await ctx.guild.unban(u); await ctx.send("✅ Débanni.")
@bot.command()
@commands.has_permissions(manage_messages=True)
async def clear(ctx, n: int): await ctx.channel.purge(limit=n+1); await send_log("CLEAR", ctx, None, f"{n} msg supprimés")
@bot.command()
...
async def tempmute(ctx, m: discord.Member, s: int):
    ...
    await ctx.send(f"🔇 {m.mention} muet {s}s."); await asyncio.sleep(s)
    for ch in ctx.guild.text_channels: await ch.set_permissions(m, send_messages=None)
    await ctx.send(f"✅ {m.mention} unmute."); await send_log("TEMPMUTE", ctx, m, f"{s}s")
@bot.command()
@commands.has_permissions(manage_roles=True)
async def unmute(ctx, m: discord.Member):
    for ch in ctx.guild.text_channels: await ch.set_permissions(m, send_messages=None)
    await ctx.send(f"✅ {m.mention} unmute."); await send_log("UNMUTE", ctx, m)
...
@bot.command()
@commands.has_role(GERANT_STAFF_ID)
async def rename(ctx, *, name: str): await ctx.channel.edit(name=name)
@bot.command()
@commands.has_role(GERANT_STAFF_ID)
async def add(ctx, m: discord.Member): await ctx.channel.set_permissions(m, view_channel=True, send_messages=True); await ctx.send("✅")
@bot.command()
@commands.has_role(GERANT_STAFF_ID)
async def remove(ctx, m: discord.Member): await ctx.channel.set_permissions(m, view_channel=False); await ctx.send("✅")

@bot.command()
@commands.has_permissions(administrator=True)
async def rank_t(ctx, m: discord.Member): await m.add_roles(ctx.guild.get_role(R_T), ctx.guild.get_role(ROLE_STAFF)); await send_log("RANK-T", ctx, m); await ctx.send(f"✅ {m.mention} Modo Test.")
@bot.command()
@commands.has_permissions(administrator=True)
async def rank_c(ctx, m: discord.Member): await m.remove_roles(ctx.guild.get_role(R_T)); await m.add_roles(ctx.guild.get_role(R_C)); await send_log("RANK-C", ctx, m); await ctx.send(f"✅ {m.mention} Modo Confirmé.")
@bot.command()
@commands.has_permissions(administrator=True)
async def rank_plus(ctx, m: discord.Member): await m.remove_roles(ctx.guild.get_role(R_C)); await m.add_roles(ctx.guild.get_role(R_PLUS)); await send_log("RANK-PLUS", ctx, m); await ctx.send(f"✅ {m.mention} Modo Plus.")
@bot.command()
@commands.has_permissions(administrator=True)
async def rank_s(ctx, m: discord.Member): await m.remove_roles(ctx.guild.get_role(R_PLUS)); await m.add_roles(ctx.guild.get_role(R_SENIOR)); await send_log("RANK-SENIOR", ctx, m); await ctx.send(f"✅ {m.mention} Modo Senior.")
@bot.command()
@commands.has_permissions(administrator=True)
async def rank_admin(ctx, m: discord.Member): await m.add_roles(ctx.guild.get_role(R_ADMIN)); await send_log("RANK-ADMIN", ctx, m); await ctx.send(f"✅ {m.mention} Admin.")
@bot.command()
@commands.has_permissions(administrator=True)
async def derank(ctx, m: discord.Member):
    for r in [ROLE_STAFF, R_T, R_C, R_PLUS, R_SENIOR, R_ADMIN]: role = ctx.guild.get_role(r); await m.remove_roles(role)
    await send_log("DERANK", ctx, m, "Destitué"); await ctx.send(f"🐉 {m.mention} est maintenant un simple mortel.")
